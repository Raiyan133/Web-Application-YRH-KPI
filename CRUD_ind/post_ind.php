...
<button class="w3-button w3-round-large w3-dark-gray" align="center" type="button" name="action_ind" id="action_ind"></button>
...
<script>  
 $(document).ready(function(){  
      ...
      {  
           var action_ind = "select_ind";  
           $.ajax({  
                url : "select_ind.php",  
                method:"POST",  
                data:{action_ind:action_ind},  
                success:function(data){  
                     ...
                     $('#last_name').val('');
                     $('#type').val('');
                     $('#action_ind').text('บันทึก');
                     $('#result').html(data);  
                }  
           });  
      }  
      $('#action_ind').click(function(){ 
           ...
           var type = $('#type').val();  
           var id = $('#num_id').val();  
           var action_ind = $('#action_ind').text(); 
           ...
           {  
                ...
                $.ajax({  
                     url : "action_ind.php",  
                     method:"POST",  
                     data:{ind_id:ind_id, Name:Name, firstName:firstName, lastName:lastName, type:type, id:id, action_ind:action_ind},  
                     success:function(data){  
                          alert(data);  
                          fetchIndicator();  
                     }  
      ...
      $(document).on('click', '.update', function(){  
           ...
           $.ajax({  
                ...
                dataType:"json",  
                success:function(data){  
                     $('#action_ind').text("บันทึกการแก้ไข");  
                     $('#num_id').val(id);  
      ...
      $(document).on('click', '.delete', function(){  
           ...
           {  
                ...
                $.ajax({  
                     url:"action_ind.php",  
                     method:"POST",  
                     data:{id:id, action_ind:action_ind},  
                     success:function(data)  
                     {  
                          fetchIndicator();  
                          alert(data);  
