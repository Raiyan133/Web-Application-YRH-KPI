<!-- Sidebar -->
<ul class="navbar-nav bg-gradient-secondary sidebar sidebar-dark accordion " id="accordionSidebar">

<!-- Sidebar - Brand -->
<a class="sidebar-brand d-flex align-items-center justify-content-center" href="admin_home.php">
  <div class="sidebar-brand-text mx-3">KPI YRH</div>
</a>

<!-- Divider -->
<hr class="sidebar-divider my-0">

<!-- Nav Item - หน้าแรก -->
<li class="nav-item">
  <a class="nav-link" href="admin_home.php">
    <i class="fas fa-fw fa-home"></i>
    <span>หน้าแรก</span></a>
</li>

<!-- Divider -->
<hr class="sidebar-divider my-0">

<!-- Nav Item - จัดการหน้าแรก -->
<li class="nav-item">
  <a class="nav-link" href="admin_managehome.php">
    <i class="fas fa-fw fa-cog"></i>
    <span>จัดการหน้าแรก</span></a>
</li>

<!-- Divider -->
<hr class="sidebar-divider">

<!-- Heading -->
<div class="sidebar-heading">
  สมาชิก
</div>

<!-- Nav Item - จัดการสมาชิก -->
<li class="nav-item">
  <a class="nav-link collapsed" href="#" data-toggle="collapse" data-target="#collapseOne" aria-expanded="true" aria-controls="collapseOne">
    <i class="fas fa-fw fa-user"></i>
    <span>จัดการสมาชิก</span>
  </a>
  <div id="collapseOne" class="collapse" aria-labelledby="headingPages" data-parent="#accordionSidebar">
    <div class="bg-white py-2 collapse-inner rounded">
      <a class="collapse-item" href="admin_staff.php">ดูสมาชิก</a>
      <a class="collapse-item" href="admin_staff.php">แก้ไขสมาชิก</a>
      <a class="collapse-item" href="admin_staff.php">ลบสมาชิก</a>
  </div>
</li>

<!-- Divider -->
<hr class="sidebar-divider">



<!-- Heading -->
<div class="sidebar-heading">
  ตัวชี้วัด
</div>

<!-- Nav Item - จัดการตัวชี้วัด -->
<li class="nav-item">
  <a class="nav-link collapsed" href="" data-toggle="collapse" data-target="#collapseTwo" aria-expanded="true" aria-controls="collapseTwo">
    <i class="fas fa-fw fa-list"></i>
    <span>จัดการตัวชี้วัด</span>
  </a>
  <div id="collapseTwo" class="collapse" aria-labelledby="headingTwo" data-parent="#accordionSidebar">
    <div class="bg-white py-2 collapse-inner rounded">
      
      <a class="collapse-item" href="admin_ind.php">เพิ่มตัวชี้วัดใหม่</a>
      <a class="collapse-item" href="admin_ind.php">แก้ไขตัวชี้วัด</a>
      <a class="collapse-item" href="admin_ind.php">ลบตัวชี้วัด</a>
    </div>
  </div>
</li>

<!-- Nav Item - ป้อนข้อมูลตัวชี้วัด -->
<li class="nav-item">
  <a class="nav-link" href="admin_insert.php">
    <i class="fas fa-fw fa-edit"></i>
    <span>ป้อนข้อมูลตัวชี้วัด</span></a>
</li>

<!-- Divider -->
<hr class="sidebar-divider">

<!-- Heading -->
<div class="sidebar-heading">
  กราฟ
</div>

<!-- Nav Item - จัดการกราฟ -->
<li class="nav-item">
  <a class="nav-link collapsed" href="#" data-toggle="collapse" data-target="#collapsePages" aria-expanded="true" aria-controls="collapsePages">
    <i class="fas fa-fw fa-chart-line"></i>
    <span>จัดการกราฟ</span>
  </a>
  <div id="collapsePages" class="collapse" aria-labelledby="headingPages" data-parent="#accordionSidebar">
    <div class="bg-white py-2 collapse-inner rounded">
      <a class="collapse-item" href="admin_edit.php">ดูกราฟ & บทวิเคราะห์</a>
      <a class="collapse-item" href="admin_edit.php">แก้ไขกราฟหรือบทวิเคราะห์</a>
      <a class="collapse-item" href="admin_edit.php">ลบกราฟหรือบทวิเคราะห์</a>
  </div>
</li>

<!-- Nav Item - ป้อนข้อมูลตัวชี้วัด -->
<li class="nav-item">
  <a class="nav-link" href="admin_compare.php">
    <i class="fas fa-fw fa-chart-area"></i>
    <span>เปรียบเทียบกราฟ</span></a>
</li>

<!-- Divider -->
<hr class="sidebar-divider">

<!-- Sidebar Toggler (Sidebar) -->
<div class="text-center d-none d-md-inline">
  <button class="rounded-circle border-0" id="sidebarToggle"></button>
</div>

</ul>
<!-- End of Sidebar -->
